...
void unir(vector<Nodo*>& heap1, vector<Nodo*>& heap2) {
    // Paso 1: Fusionar las listas ordenadas por grado
    vector<Nodo*> merged;
    ...
    merged.insert(merged.end(), it1, heap1.end());
    merged.insert(merged.end(), it2, heap2.end());

    // Paso 2: Consolidación usando vector de grados
    vector<Nodo*> grados(64, nullptr);
    for (auto nodo : merged) {
        Nodo* actual = nodo;
        while (actual) {
            Nodo* sig = actual->hermano;
            actual->hermano = nullptr; // Resetear hermano
            int grado = actual->grado;
            while (grados[grado]) {
                actual = unirArboles(actual, grados[grado]);
                grados[grado] = nullptr;
    ...
    // Reconstruir el heap desde los grados
    heap1.clear();
    for (auto& arbol : grados) {
        if (arbol) {
            heap1.push_back(arbol);
        }
    }
    heap2.clear();
}
