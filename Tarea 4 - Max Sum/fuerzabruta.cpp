...
int calcularSumaSubmatrizConPrefijo(int fila_superior, int col_izquierda, int fila_inferior, int col_derecha, vvf& pref) {
    int suma = pref[fila_inferior][col_derecha];
    if (col_izquierda - 1 >= 0) {
        suma -= pref[fila_inferior][col_izquierda - 1];
    }
    if (fila_superior - 1 >= 0) {
        suma -= pref[fila_superior - 1][col_derecha];
    }
    if (fila_superior - 1 >= 0 && col_izquierda - 1 >= 0) {
        suma += pref[fila_superior - 1][col_izquierda - 1];
    }

    return suma;
}

tuple<int, pair<int, int>, pair<int, int>> encontrarSubmatrizSumaMaximaOptimizada(vvf& matriz_entrada) {
    int filas = matriz_entrada.size();
    int columnas = matriz_entrada[0].size();
    vvf pref(filas, vf(columnas, 0.0f));

    for (int i = 0; i < filas; i++) {
        for (int j = 0; j < columnas; j++) {
            pref[i][j] = matriz_entrada[i][j];
            if (j - 1 >= 0) {
                pref[i][j] += pref[i][j - 1];
            }
        }
    ...
    pair<int, int> esquina_sup_izq = {0, 0};
    pair<int, int> esquina_inf_der = {0, 0};
        ...
        for (int col_inicio = 0; col_inicio < columnas; ++col_inicio) {
            for (int fila_fin = fila_inicio; fila_fin < filas; ++fila_fin) {
                for (int col_fin = col_inicio; col_fin < columnas; ++col_fin) {
                    ...
                    int suma_actual = calcularSumaSubmatrizConPrefijo(fila_inicio, col_inicio, fila_fin, col_fin, pref);
    ...
    return make_tuple(suma_maxima, esquina_sup_izq, esquina_inf_der);
}
...
int main() {
    ...
    for(int i = 0; i < valores_N.size(); i++) {
        ...
        for(int j=0; j<20; j++){
            ...
            auto [suma_max_val, esquina_sup_izq, esquina_inf_der] = encontrarSubmatrizSumaMaximaOptimizada(matriz);
