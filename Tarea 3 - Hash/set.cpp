...
void ejecutar_experimento(ull M_inicial, vector<int>& claves_insercion, vector<int>& claves_busqueda, vector<int>& claves_eliminacion) {
    ...
    for (int clave : claves_insercion) {
        auto [it, inserted] = ht.insert(clave);
        if (inserted) {
...
void experimento2(unsigned seed, int num_claves, ull M_inicial) {
    ...
    unordered_set<int> ht;
    ht.reserve(M_inicial);
    for(int clave : claves_insercion_inicial) ht.insert(clave);
...
void experimento4(unsigned seed, int num_operaciones_totales, ull M_inicial) {
    ...
    for (int i = 0; i < num_operaciones_totales; ++i) {
        ...
        if (op_tipo == 1) {
            if (idx_claves_disponibles < claves_disponibles.size()) {
                clave_operar = claves_disponibles[idx_claves_disponibles++];
                auto [it, inserted] = ht.insert(clave_operar);
                if (inserted) {
                    count_inserciones++;
                }
            }
            ...
            if (!ht.empty()) {
                auto it_rand = ht.begin();
                std::advance(it_rand, generarNumeroAleatorio_main(seed + i, 0, ht.size() - 1));
                clave_operar = *it_rand;

                ht.find(clave_operar);
                count_busquedas++;
            } else if (idx_claves_disponibles < claves_disponibles.size()) {
